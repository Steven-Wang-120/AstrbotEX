...
class MockScanProvider(PluginLifecycleMixin):
    id = "mock_scan"
    name = "Mock Scan Provider"
    ...
    def get_scan(self) -> ScanResult:
        ...
        return ScanResult(
            frame_id=self.frame_id,
            timestamp=time(),
            angle_min_rad=radians(-5.0),
            angle_max_rad=radians(5.0),
            angle_increment_rad=radians(1.0),
            ranges=[inf, inf, inf, inf, inf, 2.0, 1.5, 2.4, inf, inf, inf],
            range_min_m=0.05,
            range_max_m=12.0,
        )


class FailingScanProvider(PluginLifecycleMixin):
    id = "failing_scan"
    name = "Failing Scan Provider"
